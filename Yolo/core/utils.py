...
def recognize_plate(img, coords):
    ...
    box = img2[int(ymin)-5:int(ymax)+5, int(xmin)-5:int(xmax)+5]
    # grayscale region within bounding box
    # cv2.imshow("orginal", box)
    # cv2.waitKey(0)
    # k = cv2.waitKey(0)
    # k == ord('q')
...
def load_input(image_64encode):
    ...
    img = imread(BytesIO(base64.b64decode(image_64encode)))
